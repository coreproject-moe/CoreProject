import { c as create_ssr_component, l as each, e as escape } from "./ssr.js";
const latest_animes = [
  {
    id: 1,
    mal_id: 38e3,
...
const Core_project = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  const core_mapping = {
    c: "text-surface-100",
    o: "text-warning-500",
    r: "text-surface-100",
  ...
  return `<div class="flex select-none gap-[0.25vw] text-[1.5vw] font-bold leading-none"><p>${each("core".split(""), (letter) => {
    return `<span class="${"inline-flex " + escape(core_mapping[letter], true)}">${escape(letter)}</span>`;
  })}</p> <p>${each("project".split(""), (letter) => {
    return `<span class="inline-flex text-white">${escape(letter)}</span>`;
  })}</p></div>`;
});
export {
  Core_project as C,
