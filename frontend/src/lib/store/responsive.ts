...
const handleWindowChange = async (e: MediaQueryList) => {
	// Check if the media query is true
	return e.matches;
};

const checkMode = () => {
	if (handleWindowChange(MOBILE)) return "mobile";
	else if (handleWindowChange(TABLET)) return "tablet";
	else if (handleWindowChange(DESKTOP)) return "desktop";
	else if (handleWindowChange(WIDESCREEN)) return "widescreen";
	else if (handleWindowChange(FULLHD)) return "fullhd";
};
