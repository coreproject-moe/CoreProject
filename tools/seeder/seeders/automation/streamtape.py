...
async def get_upload_url(folder_id: str = STREAMTAPE_PARENT_FOLDER_ID) -> str:
    async with httpx.AsyncClient() as client:
...
async def upload_to_streamtape(file_path: str):
    upload_url = await get_upload_url()
    return await upload(upload_url, file_path)
...
#     uploaded_file_id = await upload(upload_url, file_path="1.mp4")
