...
function g(o) {
    document.body.querySelectorAll('[flamethrower-preserve]').forEach((t) => {
        let r = o.body.querySelector(
            '[flamethrower-preserve][id="' + t.id + '"]'
        );
        if (r) {
...
function A(o) {
    const e = (i) =>
            Array.from(i.querySelectorAll('head>:not([rel="prefetch"]')),
        t = e(document),
        r = e(o),
...
class q {
            ...
            (this.observer ||
                (this.observer = new IntersectionObserver((t, r) => {
                    t.forEach((n) => {
                        ...
                        n.isIntersecting &&
                            (this.createLink(s), r.unobserve(n.target));
                    });
                }, e)),
    ...
    async reconstructDOM({ type: e, next: t, prev: r, scrollId: n }) {
        ...
        try {
            if (
                ...
                this.opts.log && console.time('\u23F1\uFE0F'),
                    window.dispatchEvent(
                        new CustomEvent('flamethrower:router:fetch')
                    ),
                    e != 'popstate' && E(t);
                const i = await (
                ...
                A(c),
                    ...
                    document.createDocumentTransition
                        ...
                        : (g(c), b(), m(e, n)),
                    window.dispatchEvent(
                        new CustomEvent('flamethrower:router:end')
                    ),
                    setTimeout(() => {
                        this.prefetch();
                    }, 200),
            ...
            return (
                window.dispatchEvent(
                    new CustomEvent('flamethrower:router:error', s)
                ),
                this.opts.log && console.timeEnd('\u23F1\uFE0F'),
