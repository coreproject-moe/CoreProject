# Source = https://github.com/davidkrantz/Colorfy/blob/69e8573840df6ac9090f346256157015166620cb/spotify_background_color.py
# MIT LICENSE
import numpy as np
...
class KMeansColorAnalyzer:
    ...
    def __init__(
        ...
        img: NDArray,
        format="RGB",
        image_processing_size: tuple[int, int] | None = None,
    ) -> None:
        """
        ...
        if format == "RGB":
            ...
            raise ValueError(
                "Invalid format. Only RGB and BGR image ",
                "format supported.",
            )
    ...
    def best_color(
        ...
        colorfulness = [
            self.colorfulness(color[0], color[1], color[2]) for color in centroids
        ]
    ...
    def find_histogram(self, clt) -> float:
    ...
    def colorfulness(self, red: int, green: int, blue: int) -> float:
